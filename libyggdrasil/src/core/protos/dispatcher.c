...
static void* dispatcher_receiver(void* args) {
	...
	printf("dispatcher receiver: %lu\n", pthread_self());
	pthread_setname_np( pthread_self(), "Dispatcher-Receiver" );
...
static void* dispatcher_main_loop(main_loop_args* args) {
	...
	pthread_setname_np( pthread_self(), "Dispatcher-Sender" );
...
char number[10];
	...
	while(1){
		...
		if(elem.type == YGG_MESSAGE){
			...
			sprintf(number, "%d", elem.data.msg.Proto_id);
