...
//	printf("on msg in_est: %f , %f\n", in_est.val, in_est.weight);
//	printf("on msg recved_est: %f , %f\n", ((nei_info*)neighbour->attribute)->recved.val, ((nei_info*)neighbour->attribute)->recved.weight);

	subtract_est(&in_est, &((nei_info*)neighbour->attribute)->recved, &diff);

//	printf("on msg diff: %f , %f\n", diff.val, diff.weight);
//	printf("on msg my_est bef: %f , %f\n", state->my_est.val, state->my_est.weight);
	sum_est(&state->my_est, &diff, &state->my_est);
//	printf("on msg my_est aft: %f , %f\n", state->my_est.val, state->my_est.weight);
...
//		printf("on timer my_est: %f , %f\n", state->my_est.val, state->my_est.weight);
//		printf("on timer tmp_est: %f , %f\n", tmp_est.val, tmp_est.weight);
...
//	char id[37];
//	bzero(id, 37);
//	uuid_unparse(tosend->id, id);
//	printf("sending msg to %s\n", id);
//
	if(state->my_est.weight >= 2*q){
//		printf("sending msg with new value\n");
		est tmp_est;
...
static void process_event(YggEvent* event, limosense_state* state) {
	if(event->proto_origin == state->discovery_id){
		...
		if(event->notification_id == NEIGHBOUR_UP){
			...
			if(torm != NULL){
//				printf("my_est: %f , %f\n", state->my_est.val, state->my_est.weight);
//				printf("nei_sent: %f , %f\n", ((nei_info*)torm->attribute)->sent.val, ((nei_info*)torm->attribute)->sent.weight);
//				printf("nei_recved: %f , %f\n", ((nei_info*)torm->attribute)->recved.val, ((nei_info*)torm->attribute)->recved.weight);
//				printf("unrecved: %f , %f\n", state->unrecved.val, state->unrecved.weight);
