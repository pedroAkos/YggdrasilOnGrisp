	...
	if(!state->stable){   //|| state->stable_counter > THREASHOLD) {
		...
		while(it != NULL){
			...
			if(info->status == ACTIVE) {
				...
				YggMessage_addPayload(&msg, (void*) &info->last_acked_epoch, sizeof(unsigned short));
...
//					printf("%s has not acked my state ep: %d state->epoch: %d\n", src, ep, state->epoch);
				}
				nei->last_acked_epoch = ep;
				if(lep != nei->epoch) { //I have not acked his current state
					//printf("nei->ep %d lep: %lep\n", )
//					printf("I have not acked %s lep: %d nei->epoch: %d\n", src, lep, nei->epoch);
					state->stable = false;
				}
...
//			printf("%s has new value %f , %d\n", src, *(double*)nei->value, *(int*)(nei->value + sizeof(double)));
		}
