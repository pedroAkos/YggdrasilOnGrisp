...
static void process_request(YggRequest* request, gap_state* state) {
	...
	if(request->request == REQUEST){
		...
		if(request->request_type == AGG_GET){
			...
			if(state->parent)
				YggRequest_addPayload(request, state->parent->weight, sizeof(double));
			else
				YggRequest_addPayload(request, state->self->weight, sizeof(double));
			...
			YggRequest_freePayload(request);
			*/
			if(state->parent && state->parent->weight)
				report_aggregation(request, state->proto_id, state->self->weight, state->parent->weight, state->operation);
			else
