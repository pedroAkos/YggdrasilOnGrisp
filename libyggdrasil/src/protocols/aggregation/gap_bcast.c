...
static void updatevector(update_vector* v, gap_state* state){
	...
	//update the value that will be sending up the tree
	if(state->parent != NULL && state->parent->level < 0) {
		...
		state->root_weight = state->parent->weight;
	}

	if(state->root_weight != NULL) {
		v->root_weight = malloc(state->val_size);
		memcpy(v->root_weight, state->root_weight, state->val_size);
	} else
		v->root_weight = NULL;
}
...
static void addVectorToMsg(YggMessage* msg, update_vector* v, gap_state* state){
	...
	if(v->root_weight != NULL)
		YggMessage_addPayload(msg, v->root_weight, state->val_size);
}
...
static void process_all(gap_state* state) {
	...
	//if((state->vector == NULL || !comp_vector(state->vector, newvector)) &&
	if(state->timeout){

		broadcast_update_vector(newvector, state);
		if(state->vector != NULL){
			if(state->vector->root_weight)
				free(state->vector->root_weight);
		...
		if(newvector->root_weight)
			free(newvector->root_weight);
...
static void deserialize_vector(update_vector* other, YggMessage* msg, gap_state* state) {
	...
	if(len >= sizeof(uuid_t)){ //before len > 0
		payload += sizeof(int);

		memcpy(other->parent, payload, sizeof(uuid_t));
		len -= sizeof(uuid_t);
		if(len >= state->val_size) { //before len > 0
			payload += sizeof(uuid_t);

			other->root_weight = malloc(state->val_size);
			memcpy(other->root_weight, payload, state->val_size);
		} else
			other->root_weight = NULL;
	}
}

static void updateentry(uuid_t n, WLANAddr addr, void* w, int l, uuid_t p, void* r_w, gap_state* state){
	...
	if(r_w && state->parent && uuid_compare(n, state->parent->neigh_id) == 0) {
		if(!state->root_weight)
			state->root_weight = malloc(state->val_size);

		memcpy(state->root_weight, r_w, state->val_size);
	}
}
...
static void process_msg(YggMessage* msg, gap_state* state) {
	...
	updateentry(other.neigh, msg->srcAddr, other.weight, other.level, other.parent, other.root_weight, state);

	free(other.weight);
	other.weight = NULL;
	if(other.root_weight)
		free(other.root_weight);
...
static void process_request(YggRequest* request, gap_state* state) {
	...
	if(request->request == REQUEST){
		...
		if(request->request_type == AGG_GET){
			...
			//TODO change to report_aggregation
			/*
			...
			YggRequest_addPayload(request, state->self->weight,  sizeof(double));
			if(state->root_weight)
				YggRequest_addPayload(request, state->root_weight, sizeof(double));
			else
				YggRequest_addPayload(request, state->self->weight, sizeof(double));
			...
			*/
			if(state->root_weight)
				report_aggregation(request, state->proto_id, state->self->weight, state->root_weight, state->operation);
			else
				report_aggregation(request, state->proto_id, state->self->weight, state->self->weight, state->operation);
...
proto_def* gap_bcast_init(void* args) {
	...
	state->proto_id = PROTO_AGG_GAP_BCAST;
	state->operation = gargs->aggregation_operation;
	...
	proto_def* gap = create_protocol_definition(state->proto_id, "GAP BCAST", state, (Proto_destroy) gap_destroy);
