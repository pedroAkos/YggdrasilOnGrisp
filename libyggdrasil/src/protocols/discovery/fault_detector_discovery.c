...
static void process_up_stream_msg(YggMessage* msg, fdd_state* state) {
	...
	if(read == sizeof(uuid_t)) {
		...
		if(nei == NULL) { //new nei
			...
			//ygg_log("FAULT DETECTOR DISCOVERY", "UPDATED NODE", uuid_str);
	...
	if(msg->Proto_id != state->proto_id) {
		...
		char log_msg[50];
		bzero(log_msg, 50);
		sprintf(log_msg, "to be delivered to: %d", msg->Proto_id);
		ygg_log("FDD", "ALIVE", log_msg);
		#endif
...
static void* fd_discov_main_loop(main_loop_args* args) {
	...
	pthread_setname_np( pthread_self(), "Discovery" );
	...
	while(true) {
		...
		case YGG_TIMER:
			...
			if(state->announce && uuid_compare(t->id, state->announce->id) == 0) {
				...
				YggTimer_set(&elem.data.timer, state->announce_period_s, state->announce_period_ns*(rand()%(state->database->size+1)), 0, 0);
...
proto_def* fault_detector_discovery_init(void* args) {
	...
	YggTimer_set(state->announce, 1, 0, 0,0);//state->announce_period_s, state->announce_period_ns);
